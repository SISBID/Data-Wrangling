...
```{r RawlMatch}
grep("two aliens",ufo$comments)[1:5]
which(grepl("two aliens", ufo$comments))[1:5]
...
```{r RawlMatch_log}
head(grepl("two aliens", ufo$comments))
head(str_detect(ufo$comments, "two aliens"))
```
...
```{r grepl}
grep("two aliens", ufo$comments,value=TRUE)[1:3]
ufo[grep("two aliens",ufo$comments)[1:3],]
```
