...
```{r}
ufo_clean %>% mutate(country = case_when(country, gb = "Great Britain",
                                               "us" = "United States")) %>% glimpse()
...
ufo_clean <- ufo_clean %>% mutate(prob_country = 
      case_when((is.na(country) & state %in% c(US_states)) ~ "United States",
                ...
                (is.na(country) & state %in% c(de_states)) ~ "Germany",
                (is.na(country) & state %in% c(de_states)) ~ "Great Britain",
                   TRUE ~ country))
```
...
We would want to confirm what we recoded with the cities and latitude and longitude. 
...
ufo_clean <- ufo_clean %>% mutate(prob_country = 
      case_when(
      (is.na(country) & state %in% c(US_states)) & !is.na(state) | country == "us" ~ "United States",
      (is.na(country) & state %in% c(CA_states)) & !is.na(state)  | country == "ca" ~ "Canada",
      (is.na(country) & state %in% c(au_states)) & !is.na(state)  | country == "au" ~ "Australia",
      (is.na(country) & state %in% c(de_states)) & !is.na(state)  | country == "de" ~ "Germany",
      (is.na(country) & state %in% c(de_states)) & !is.na(state)  | country == "gb" ~ "Great Britain",
                   TRUE ~ country))
