...
`across` allows us to perform functions on specific columns more easily.
...
If you want to use arguments, then you need to use the `~`  and `.x` (or `.`) as a place holder for what you will be passing into the function.

See [purrr-shortcuts](https://adv-r.hadley.nz/functionals.html#purrr-shortcuts) and [this link about `.` and `.x`](https://forum.posit.co/t/function-argument-naming-conventions-x-vs-x/7764/10).
...
- an example would be improting multiple files with a vector of different delimiters
...
Both `map_df` and `across` can do the same thing, `map_df` is simpler, but `across` keeps rownames.
