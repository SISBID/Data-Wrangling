...
## Sometime data class gets in the way{.codesmall}

```{r}
ufo |> filter(datetime >2014) |> head(2) # not right!
```


## Putting it all together{.codesmall}
Note there is a warning if you run this.  More on this a bit later.
More advanced example.

```{r, warning=FALSE}
ufo |> mutate(datetime = dmy_hm(datetime)) |> # match format to change - lubridate function
...
[Link to Lab](http://sisbid.github.io/Data-Wrangling/05_Data_Subsetting/lab/data-subsetting-lab-part2.Rmd)
