...
1) Look at the data
1) Recode variable names as needed
1) Check for classes of variables (update as needed)
1) Check for missing data (recode as needed)
1) Check for duplicates (aggregate as needed)
1) Check for rare values (recode as needed)
1) Recode values as needed

## Read in the UFO dataset{.codesmall}

Read in data or download from: http://sisbid.github.io/Data-Wrangling/data/ufo/ufo_data_complete.csv.gz

This data is originally from kaggle although it was recently renamed or removed. 
...
But our other problems seem to be gone!
...
## Check one of our previously problematic rows:

Looks like the value of 2` is there now!
...
We could get the shifted rows, rename the columns to be correct and then join this with the existing data if we wanted to try to keep all rows. We will learn about merging data later. 
...
```
## Taking a look at the shifted rows {.codesmall}

Indeed they do look shifted with shapes not in the `shape` column.
...
## Exact Swaps - `case_when` function{.codesmall}
...
Watch out for parentheses placement! 

Here the parentheses are in the wrong place so the `.default` statement isn't used.

Be sure to check the data before and after.
...
## `case_when()` to create or modify a variable based on conditions of other variables{.codesmall}

```
case_when(test ~ value if test is true,
         test2 ~ value if test2 is true,
        .default ~ value if all above tests are not true) # defaults to NA
```
...
- `case_when` can use conditionals, need to specify what value for if no conditions are met using `.default`(can be the original value of a variable if we use the variable name).
